...
class HuggingfaceClassifier(Classifier):
    ...
    def __init__(
        ...
        max_length: int = 1024,
        rand_order_evidence: bool = False,
        num_orders: int = 10,
        seed: int = 42,
    ) -> None:
        super().__init__()
    ...
    def __call__(
        ...
        **kwargs,
        ...
        inputs = self.tokenizer(
            claims, evidences, return_tensors="pt", padding=True, truncation=True, max_length=self.max_length
        )
        ...
        return ClassificationResult(
            probs=probs, metadata={"logits": logits.cpu().numpy(), "mean_logits": mean_logits.cpu().numpy()}
        )
...
class RandomForestClassifier(Classifier):
    ...
    def predict_single_claim(self, claim):
        ...
        if self.main_label_map_inverse[predicted_label] in ["Supported", "Refuted"] and confidence >= 0.65:
            ...
            class_probabilities = {
                self.main_label_map_inverse[i]: prob for i, prob in enumerate(probability_distribution)
            }
        else:
            ...
            class_probabilities = {
                self.secondary_label_map_inverse[i]: prob for i, prob in enumerate(probability_distribution)
            }
...
class AverageEnsembleClassifier(Classifier):
    ...
    def __init__(self, classifiers: List[Classifier], weights: np.ndarray = None):
        ...
        assert (
            len(classifiers) == len(weights) if weights is not None else True
        )  # check if number of classifiers and weights match
    ...
    def __call__(self, datapoint, evidence_generation_result, retrieval_result):
        clf_probs = [
            c(datapoint, evidence_generation_result, retrieval_result).probs for c in self.classifiers
        ]
        return ClassificationResult(
    ...
    def fit_weights(
        self, datapoints, evidence_generation_results, retrieval_results, metric: str = "cross-entropy"
    ):
        ...
        for clf in self.classifiers:
            clf_predictions = []
            for datapoint, evidence_generation_result, retrieval_result in zip(
                datapoints, evidence_generation_results, retrieval_results
            ):
        ...
        if len(self.classifiers) == 1:
            ...
            res = opt.minimize(
                opt_func_multivariate, self.weights, method="SLSQP", bounds=bounds, constraints=cons
            )
...
class LogRegEnsembleClassifier(Classifier):
    ...
    def __call__(self, datapoint, evidence_generation_result, retrieval_result):
        ...
        return ClassificationResult(
            probs=self.logreg.predict_proba(input).squeeze(), metadata={"clf_probs": clf_probs}
        )
