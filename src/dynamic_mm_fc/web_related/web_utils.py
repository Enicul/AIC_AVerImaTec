...
def is_image_url(url):
    ...
    try:
        ...
        return (
            ...
            and not "svg" in response.headers.get("content-type")
            and not "svg" in content_type
            and not "eps" in content_type
        )
    except Exception:
        return False
        ...
        if is_image_url(url) and not is_fact_checking_site(url) and not is_unsupported_site(url):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.write(
                        "black",
                        "\tHaving image!! and saving at %s, the %d image" % (path_to_file, media_count),
                    )
...
def scrape_firecrawl(url):
    ...
    try:
        response = requests.post(
            firecrawl_url + "/v1/scrape", json=json_data, headers=headers, timeout=10 * 60
        )  # Firecrawl scrapes usually take 2 to 4s, but a 1700-page PDF takes 5 min
