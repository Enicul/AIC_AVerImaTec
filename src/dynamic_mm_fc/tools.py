...
def google_search_text(search_term, search_type, **kwargs):
    ...
    if search_type == "text":  # text query for text
        ...
        res = (
            service.cse()
            .list(q=search_term, searchType="image", cx=GOOGLE_SEARCH_ENGINE_ID, **kwargs)
            .execute()
        )
    if "items" in res:
...
def det_web_valid_filter(image, date):
    ...
    for page in web_annotations.pages_with_matching_images:
        ...
        cur_raw_ris_result.append({"title": title, "url": page.url, "date": page_date})
...
def scrap_from_ris_results(valid_web_urls):
    ...
    for ris_result in valid_web_urls:
        ...
        all_texts.append({"scrape_content": scraped_result, "url": url})
    return all_texts
...
def gen_answer_with_llm(detailed_evid, ques_txt, answer_llm, answer_llm_name):
    ...
    if "gemini" in answer_llm_name:
        inputs = [inputs]
        response = answer_llm.models.generate_content(model=answer_llm_name, contents=inputs)
        ...
        generated_ids = answer_llm["model"].generate(**model_inputs, max_new_tokens=256)
        generated_ids = [
        ...
        response = answer_llm["processor"].batch_decode(generated_ids_trimmed, skip_special_tokens=True)[0]
        answer = response.replace("**ANSWER:**", "").strip()
    return answer
...
def reverse_image_search(
    ...
    for i, image in enumerate(image_lists):
        ...
        if use_data_store:
            ...
            valid_web_urls = load_json(
                os.path.join(path, "web_det_info/reverse_image_search/urls", img_name + ".json")
            )
        else:
            """
            ...
            json.dump(
                valid_web_urls,
                open(os.path.join(path, "web_det_info/reverse_image_search/urls", img_name + ".json"), "w"),
            )
        if use_data_store:
            all_texts = []
            datastore_file = os.path.join(web_text_path, str(req_id) + ".json")
            saved_kb = load_jsonl(datastore_file)
            for row in saved_kb:
                all_texts.append({"scrape_content": " ".join(row["url2text"]), "url": row["url"]})
        elif os.path.exists(
            os.path.join(path, "web_det_info/reverse_image_search/scrap", img_name + ".json")
        ):
            all_texts = load_json(
                os.path.join(path, "web_det_info/reverse_image_search/scrap", img_name + ".json")
            )
            ...
            json.dump(
                all_texts,
                open(os.path.join(path, "web_det_info/reverse_image_search/scrap", img_name + ".json"), "w"),
            )
        if use_data_store == False or (use_data_store and i == 0):
    ...
    try:
        answer = gen_answer_with_llm(detailed_evid, ques_txt, answer_llm, answer_llm_name)
    except:
...
def vqa_mllm(
    ...
    if "gemini" in answer_mllm_name:
        ...
        for image in image_lists:
            ...
            inputs.append(im)
        response = answer_mllm.models.generate_content(model=answer_mllm_name, contents=inputs)
        answer = response.text.strip()
    elif answer_mllm_name in ["qwen", "gemma"]:
        messages = [
            {
                "role": "user",
                "content": [{"type": "text", "text": "Question: " + ques_txt}],
            }
        ]
        for image in image_lists:
        ...
        inputs = answer_mllm["processor"](
            images=pil_images, text=prompts, padding=True, return_tensors="pt"
        ).to(answer_mllm["model"].device)
        if answer_mllm_name == "llava-inter":
            inputs = inputs.to(torch.float16)

        if answer_mllm_name == "llava":
            output = answer_mllm["model"].generate(
                **inputs,
                max_new_tokens=128,
                pad_token_id=answer_mllm["processor"].tokenizer.eos_token_id,
                eos_token_id=[
                    answer_mllm["processor"].tokenizer.eos_token_id,
                    answer_mllm["processor"].tokenizer.convert_tokens_to_ids("<|eot_id|>"),
                ][0]
            )
            answer = (
                answer_mllm["processor"]
                .decode(output[0], skip_special_tokens=True, clean_up_tokenization_spaces=False)
                .split("[/INST]")[-1]
                .strip()
            )
        ...
        for image in image_lists:
            pil_images.append(Image.open(image).convert("RGB"))
        inputs = answer_mllm["processor"](images=[pil_images], text=prompt, return_tensors="pt").to(
            answer_mllm["model"].device
        )
        output = answer_mllm["model"].generate(**inputs, max_new_tokens=128)
        answer = (
            self.qg_model["processor"]
            .decode(output[0], skip_special_tokens=True)[inputs.input_ids.shape[1] :]
            .strip()
        )
...
def text_search_text(
    ...
    if use_data_store:
        ...
        for page_num in range(num_pages):
            try:
                search_results = google_search_text(
                    search_term,
                    search_type="text",
                    num=10,
                    start=0 + 10 * page_num,
            ...
            for row in search_results:
                ...
                if "title" in row:
                    ...
                    title = ""
                valid_web_urls.append({"url": row["link"], "title": title})
        json.dump(
            valid_web_urls,
            open(os.path.join(path, "web_det_info/text_search_text/urls", file_name + ".json"), "w"),
        )
    if use_data_store:
        all_texts = []
        datastore_file = os.path.join(web_text_path, str(req_id) + ".json")
        ...
        for row in saved_kb:
            all_texts.append({"scrape_content": " ".join(row["url2text"]), "url": row["url"]})
    elif os.path.exists(os.path.join(path, "web_det_info/text_search_text/scrap", file_name + ".json")):
        ...
        json.dump(
            all_texts,
            open(os.path.join(path, "web_det_info/text_search_text/scrap", file_name + ".json"), "w"),
        )
...
def gen_answer_with_mllm(detailed_evid, ques_txt, answer_mllm, answer_mllm_name):
    if "gemini" in answer_mllm_name:
        inputs = [
            "Question: "
            + ques_txt
            + " Please select the most relevant images from the image list and answer with the image index (1, 2, 3, etc) without any other words. Image indexing starts from 1."
        ]
        ...
        prompts = answer_mllm["processor"].apply_chat_template(messages, add_generation_prompt=True)
        inputs = answer_mllm["processor"](
            images=image_list, text=prompts, padding=True, return_tensors="pt"
        ).to(answer_mllm["model"].device)
        if answer_mllm_name == "llava-inter":
            inputs = inputs.to(torch.float16)
        ...
        for image in detailed_evid:
            image_list.append(Image.open(image).convert("RGB"))
        inputs = answer_mllm["processor"](images=[image_list], text=prompt, return_tensors="pt").to(
            answer_mllm["model"].device
        )
        output = answer_mllm["model"].generate(
            **inputs, max_new_tokens=128, pad_token_id=answer_mllm["processor"].tokenizer.eos_token_id
        )
        answer = (
            answer_mllm["processor"]
            .decode(output[0], skip_special_tokens=True)[inputs.input_ids.shape[1] :]
...
def text_search_image(
    ...
    file_name = search_term + "_" + sort_date
    if use_data_store:
        image_path_dir = os.path.join(datastore_path, "image_related/image_related_store_image_" + test_mode)
    elif os.path.exists(os.path.join(path, "web_det_info/text_search_image/urls", file_name + ".json")):
        valid_web_urls = load_json(
            os.path.join(path, "web_det_info/text_search_image/urls", file_name + ".json")
        )
    else:
        """
        Here how many queries set 30 as default!!!
        ...
        json.dump(
            valid_web_urls,
            open(os.path.join(path, "web_det_info/text_search_image/urls", file_name + ".json"), "w"),
        )
    # downloading images ==>search_term+str(i)
    if use_data_store:
        image_names = os.listdir(os.path.join(image_path_dir, str(req_id)))
        images = [os.path.join(os.path.join(image_path_dir, str(req_id), img)) for img in image_names]
    elif os.path.exists(os.path.join(path, "web_det_info/text_search_image/images", file_name)):
        image_names = os.listdir(os.path.join(path, "web_det_info/text_search_image/images", file_name))
        images = [
            os.path.join(os.path.join(path, "web_det_info/text_search_image/images", file_name, img))
        ...
        for i, row in enumerate(valid_web_urls):
            ...
            try:
                ...
                im.save(
                    os.path.join(
                        os.path.join(
                            path, "web_det_info/text_search_image/images", file_name, str(i) + ".jpg"
                        )
                    )
                )
                images.append(
    ...
    if len(detailed_evid) == 0:
        ...
        answer = gen_answer_with_mllm(detailed_evid, ques_txt, answer_mllm, answer_mllm_name)
    return answer, detailed_evid, evid_url
