...
class MM_Checker:
    ...
    def __init__(
        ...
        self.justification_gen = (
            justification_gen  # generating a justification based on the evidence and claim
        )
    ...
    def verify_mm_claim(self, claim_txt, claim_img, meta_date, meta_loc, req_id, gt_info=None):
        ...
        if self.gt_ques:
            ...
            questions = []
            candidate_questions = self.qg_model.generate_parallel_ques(
                claim_txt, claim_img, self.num_gen_ques, self.qg_icl
            )
            ...
            if max_iter > len(questions) and self.hybrid_qg == False:
                max_iter = len(questions)
        num_invalid = 0
        detailed_results = {"QA_info": [], "evidence": None}
        ...
        for iter in range(max_iter):
            ...
            if self.para_qg or self.no_search or self.gt_ques or (self.hybrid_qg and iter < len(questions)):
                ...
                if self.debug and self.hybrid_qg:
                    print(iter, "Dynamic##")
                ques = self.qg_model.gen_question(
                    evid_context, claim_txt, claim_img, meta_date, meta_loc, num_orig_claim_imgs, self.qg_icl
                )
            if self.debug:
            ...
            if ques is None or len(ques) < 10:
                continue
            try:
                ques_txt, img_idx, ques_type = parse_ques(
                    ques, len(detailed_results["QA_info"]), self.para_qg
                )
                if ques_txt is None or len(ques_txt.split(" ")) < 5:
                    continue
                ...
                continue
            ans, evid, ans_method = self.qa_model.answer_ques(
                claim_txt,
                claim_img,
                evid_context,
                ques_txt,
        ...
        justification = self.justification_gen.justi_gen(
            cur_verdict, meta_info, claim_txt, claim_img, evid_context, num_orig_claim_imgs
        )
...
def predict_verdict_with_gt_evid(
    verifier,
    justification_gen,
    gt_info,
    claim_txt,
    ...
    for info in gt_info:
        ...
        if len(related_images) == 0:
            ...
            evid_ques = question + ", ".join(
                ["[IMG_" + str(i + 1) + "]" for (i, _) in enumerate(related_images)]
            )
        ...
        evid = qa_to_evidence.qa_to_evid(evid_ques, evid_ans, answer_llm, answer_llm_name, path)
        print("\tconverted evid:", evid)
        evid_context.append({"text": evid, "images": related_images})
    ...
    justification = justification_gen.justi_gen(
        verdict, meta_info, claim_txt, claim_img, evid_context, num_orig_claim_imgs
    )
...
if __name__ == "__main__":
    ...
    p2_data = load_json(
        os.path.join(args.ROOT_PATH, "data/data_clean/split_data/" + args.TEST_MODE + ".json")
    )
    ...
    if "gemini" in llm_name:
        ...
        llm_model = {"pipeline": pipeline, "terminators": terminators}
    elif llm_name == "qwen":
        ...
        llm_model = {"model": model, "tokenizer": tokenizer}
    elif llm_name == "gemma":
        ...
        llm_model = {"model": model, "processor": processor}
    if "gemini" in llm_name and "gemini" in mllm_name:
        ...
        if "gemini" in mllm_name:
            ...
            model = LlavaNextForConditionalGeneration.from_pretrained(
                "llava-hf/llava-v1.6-mistral-7b-hf", torch_dtype=torch.float16, device_map={"": "cuda:1"}
            ).eval()
            ...
            model = AutoModelForVision2Seq.from_pretrained(
                "HuggingFaceM4/Idefics3-8B-Llama3", torch_dtype=torch.bfloat16, device_map={"": "cuda:1"}
            )
            mllm_model = {"model": model, "processor": processor}
            ...
            # processor.tokenizer.pad_token = "[PAD]"
            # processor.tokenizer.padding_side = "left"
            mllm_model = {"model": model.eval(), "processor": processor}
    ...
    if (
        os.path.exists(os.path.join(args.ROOT_PATH, "fc_detailed_results", "_".join([llm_name, mllm_name])))
        == False
    ):
    ...
    if args.NO_SEARCH:
        ...
        prev_pred_file = load_pkl(
            os.path.join(args.ROOT_PATH, "fc_detailed_results", "_".join([llm_name, mllm_name]), "14.pkl")
        )
    for i, annotation in enumerate(p2_data):
        ...
        if args.GT_EVID:
            try:
                verdict, justification, evid_context = predict_verdict_with_gt_evid(
                    verifier,
                    justification_gen,
                    questions,
                    claim_txt,
