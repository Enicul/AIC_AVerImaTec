...
if __name__ == "__main__":
    ...
    p2_data = load_json(os.path.join(args.root_dir, "test.json"))
    ...
    if os.path.exists(os.path.join(args.root_dir, "prepare_submission/converted_results", "gt_evid_test.json")):
        gt_evid_flag = True
        gt_evid_set = load_json(
            os.path.join(args.root_dir, "prepare_submission/converted_results", "gt_evid_test.json")
        )
    else:
        gt_evid_flag = False
        gt_evid_set = []
    ...
    for i, row in enumerate(p2_data):
        ...
        if False:
            ...
            if args.debug:
                ...
                print("##Evidence:\n", detailed_evid_val, "\n", img_scores, "\n\t", evid_acc)
                print("##Justification:\n", justi_feedback, "\n", justi_score, "\n\t", justi_acc)
        ques_score = evid_acc = verdict_acc = justi_acc = ques_feedback=ques_score = justi_feedback =justi_score = detailed_evid_val = img_scores = evid_val_score = ques_acc = 0
    ...
    if gt_evid_flag == False:
        json.dump(
            gt_evid_set,
            open(os.path.join(args.root_dir, "prepare_submission/converted_results", "gt_evid_test.json"), "w"),
        )
